...
#----------------------------------------------------------------------------
# setenv('MW_NVCC_PATH','/usr/local/cuda/bin/nvcc') in matlab to set cuda
    TMW_ROOT="$MATLAB"
    MFLAGS=''
...
#----------------------------------------------------------------------------
            ...
            CLIBS="$RPATH $MLIBS -lm -lmwgpu $TMW_ROOT/bin/$Arch/libcudart.so"
...
#
            ...
            # $NVCC was set above.
            CXX='$NVCC'
            CXXFLAGS="-gencode=arch=compute_13,code=sm_13 -gencode=arch=compute_20,code=sm_20 -gencode=arch=compute_30,code=\\\"sm_30,compute_30\\\" -I$TMW_ROOT/toolbox/distcomp/gpu/extern/include -I$CUDA_INSTALL_PATH/samples/common/inc -IUtilNPP  --compiler-options=-ansi,-D_GNU_SOURCE,-fPIC,-fno-omit-frame-pointer,-pthread"
            CXXLIBS="$RPATH $MLIBS -lm -lmwgpu $CUDA_INSTALL_PATH/lib64/libcudart.so $CUDA_INSTALL_PATH/lib64/libnppi.so $CUDA_INSTALL_PATH/lib64/libnppc.so"
