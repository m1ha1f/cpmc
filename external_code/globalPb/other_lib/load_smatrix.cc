...
mexFunction (int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[])
{
    ...
    {
      int n = 0;
      duh = fread(&n,sizeof(int),1,fp);
      int nnz = 0;
      duh = fread(&nnz,sizeof(int),1,fp);
      ...
      {
        duh = fread(&nz,sizeof(int),1,fp); //number of entries in this row
        duh = fread(&(z[ct]),sizeof(double),nz,fp); //value
        duh = fread(&(xint[ct]),sizeof(int),nz,fp);    //col index
