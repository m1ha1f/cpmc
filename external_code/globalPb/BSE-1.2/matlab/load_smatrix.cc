...
mexFunction (int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[])
{
    ...
    char *filename = (char*)mxCalloc(mxGetN(prhs[0])+1, sizeof(char)); //mxCalloc is similar to malloc in C
    ...
    if (fp != NULL)
    {
      ...
      for (int row = 0; row < n; row++) 
      {
        ...
        {
          x[ct+col] = (double)xint[ct+col] + 1; //add one for matlab indexing
          y[ct+col] = (double)row + 1;
        } 
        ct = ct + nz;
      }
      fclose(fp);
